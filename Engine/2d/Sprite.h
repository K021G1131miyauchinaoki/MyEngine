...
//アンカーポイント
		...
		const XMFLOAT2& GetAnchorPoint()const {
			return anchorPoint;
		}
// 左右反転の設定
...
//テクスチャ番号
		void	SetTexIndex(const uint32_t texIndex_) {
			texIndex = texIndex_;
		}
		const	uint32_t& GetTexIndex()const {
			return texIndex;
		}
//サイズ
		...
		const XMFLOAT2& GetTexSize()const {
			return texSize;
		}
//サイズ
		...
		const XMFLOAT2& GetTexLeftTop()const {
			return texLeftTop;
		}
